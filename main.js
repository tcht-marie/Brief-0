...
function nombreAleatoire() {
  ...
  console.log(nombre);
  return nombre; //appel de la fonction Math.random avec return
}

const number = nombreAleatoire(); // création const number pour la réutiliser plus tard
...
arrondirNombre(number); // appel de la fonction en utilisant en paramètre la const number qui appel la fonction nombreAleatoire().
...
window.addEventListener("contextmenu", function (event) {
  event.preventDefault(); // code manquant : il faut empêcher le comportement par défaut du clic droit
